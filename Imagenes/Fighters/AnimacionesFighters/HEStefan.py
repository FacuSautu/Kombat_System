...
class Player2(pilasengine.actores.Actor):
    ...
    def iniciar(self, enemigoP1):
        ...
        self.escala = .7
        self.espejado = True
        self.hacer_inmediatamente(HabilidadEspecialStefan)
...
class HabilidadEspecialStefan(pilas.comportamientos.Comportamiento):
    
    def iniciar(self, receptor):
        self.receptor = receptor
        self.control = mi_controlP1
        
        AnimacionP2 = pilas.imagenes.cargar_animacion("HabilidadesEspeciales/HEStefan/HEStefan7.png", 7, 1)
        
        AnimacionP2.definir_animacion('Primero', [0, 1, 2, 3, 5, 6], 10)
        AnimacionP2.definir_animacion('carga', [5, 6, 5, 6, 5, 6, 5, 6], 10)
            
        self.receptor.imagen = AnimacionP2
        self.receptor.centro = ("centro", "abajo")
        ...
        def caidaUSB():
            global USB
            USB = pilas.actores.Actor()
            USB.imagen = "HabilidadesEspeciales/HEStefan/ConectorUSBMod.png"
            #USB.escala = 1.4
            USB.x = EnemyP1.x + 100
            USB.y = EnemyP1.y+500
            USB.aprender('RebotarComoCaja')
            pilas.tareas.agregar(2, terminar)
        
        def caidaMouse():
            global Mouse
            Mouse = pilas.actores.Actor()
            Mouse.imagen = "HabilidadesEspeciales/HEStefan/MouseStefanMod.png"
            Mouse.escala = 2.5
            Mouse.x = EnemyP1.x
            Mouse.y = EnemyP1.y+500
            Mouse.centro = ("centro", "abajo")
            Mouse.aprender('RebotarComoCaja')
            pilas.fisica.gravedad_y=-100
            pilas.fisica.eliminar_techo()
            pilas.fisica.crear_suelo((800,400))
            pilas.tareas.agregar(3, caidaUSB)
            pilas.tareas.agregar(.5, mouseDamage)
        
        def mouseDamage():
            VidaP1.progreso -= 50
            EnergiaP2.progreso = 0
        
        def cargar():
            self.receptor.imagen.cargar_animacion('carga')
            pilas.tareas.agregar(2, caidaMouse)
            
        pilas.tareas.agregar(.5, cargar)
...
pilas.comportamientos.vincular(HabilidadEspecialStefan)


Mono = pilas.actores.Mono(x= -200, y=-100)
#Mono.figura_de_colision.sensor = False
